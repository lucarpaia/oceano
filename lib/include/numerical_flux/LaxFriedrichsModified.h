...
namespace NumericalFlux
{
      ...
      const Number                     zb_p) const
  {
    ...
    const auto hu_m = model.depth(z_m, zb_star) * v_m;
    const auto hu_p = model.depth(z_p, zb_star) * v_p;
    const auto flux_m = model.tracer_adv_flux<dim, n_tra>(z_m, hu_m, t_m, zb_star);
    const auto flux_p = model.tracer_adv_flux<dim, n_tra>(z_p, hu_p, t_p, zb_star);
      ...
      const Number                  zb_p) const
  {
    ...
    const auto hu_m = model.depth(z_m, zb_star) * v_m;
    const auto hu_p = model.depth(z_p, zb_star) * v_p;
    const auto flux_m = model.tracer_adv_flux(z_m, hu_m, t_m, zb_star);
    const auto flux_p = model.tracer_adv_flux(z_p, hu_p, t_p, zb_star);
