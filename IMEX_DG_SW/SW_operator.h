...
namespace SW {
  ...
  template<int dim, unsigned int n_stages,
           int fe_degree_zeta, int fe_degree_hu, int fe_degree_hc,
           int n_q_points_1d_zeta, int n_q_points_1d_hu, int n_q_points_1d_hc, typename Vec>
  ...
  assemble_rhs_cell_term_zeta(const MatrixFree<dim, Number>&               data,
    ...
    if(IMEX_stage <= n_stages) {
      ...
      height, 1 for discharge (aka mass flux) and 2 for tracer). ---*/
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        phi_zeta_old.reinit(cell);
        phi_zeta_old.gather_evaluate(src[0], EvaluationFlags::values);
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& zeta_old = phi_zeta_old.get_value(q);
  ...
  assemble_cell_term_zeta(const MatrixFree<dim, Number>&               data,
                          ...
                          const Vec&                                   src,
                          const std::pair<unsigned int, unsigned int>& cell_range) const {
    FEEvaluation<dim, fe_degree_zeta, n_q_points_1d_hu, 1, Number> phi(data, 0);
    ...
    if(IMEX_stage <= n_stages) {
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          for(unsigned int s = 1; s <= IMEX_stage - 1; ++s) {
            const auto& zeta_s      = phi_zeta[s - 1].get_value(q);
            const auto& hu_s        = phi_hu[s - 1].get_value(q);

            const auto& grad_zeta_s = phi_zeta[s - 1].get_gradient(q);

            flux += a[IMEX_stage - 1][s - 1]*dt*outer_product(hu_s, hu_s/(zeta_s + zb_q));
            non_cons_flux += a[IMEX_stage - 1][s - 1]*dt*EquationData::g*(zeta_s + zb_q)*grad_zeta_s;
          }
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          for(unsigned int s = 1; s <= IMEX_stage - 1; ++s) {
            ...
            flux += b[s - 1]*dt*outer_product(hu_s, hu_s/(zeta_s + zb_q));
            non_cons_flux += b[s - 1]*dt*EquationData::g*(zeta_s + zb_q)*grad_zeta_s;

            /*--- const auto& gamma_s = 0.0; TODO: Add proper expression of the friction
                friction += b_tilde[s-1]*dt*gamma_s*(hu_s/(zeta_s + zb_q)); ---*/
          }
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi_m.n_q_points; ++q) {
          ...
          for(unsigned int s = 1; s <= IMEX_stage - 1; ++s) {
            ...
            const auto& avg_tensor_product_flux = 0.5*(outer_product(hu_s_m, hu_s_m/(zeta_s_m + zb_q)) +
                                                       outer_product(hu_s_p, hu_s_p/(zeta_s_p + zb_q)));
            const auto& lambda_s                = std::max(std::abs(scalar_product(hu_s_m/(zeta_s_m + zb_q), n_plus)) +
                                                           std::sqrt(EquationData::g*(zeta_s_m + zb_q)),
                                                           std::abs(scalar_product(hu_s_p/(zeta_s_p + zb_q), n_plus)) +
                                                           std::sqrt(EquationData::g*(zeta_s_p + zb_q)));
            ...
            /*const auto& avg_flux_part_1_zb      = 0.5*EquationData::g*((zeta_s_m + zb_q)*zeta_s_m + (zeta_s_p + zb_q)*zeta_s_p);
            const auto& avg_flux_part_2_zb      = 0.5*EquationData::g*((zeta_s_m + zb_q) + (zeta_s_p + zb_q));*/ //NOTE: No exact cancellation
            const auto& avg_flux_part_1_zb      = 0.5*EquationData::g*(zeta_s_m + zb_q)*zeta_s_m
                                                + 0.5*EquationData::g*(zeta_s_p + zb_q)*zeta_s_p;
            const auto& avg_flux_part_2_zb      = 0.5*EquationData::g*(zeta_s_m + zb_q)
                                                + 0.5*EquationData::g*(zeta_s_p + zb_q);
  ...
  assemble_cell_term_hu(const MatrixFree<dim, Number>&               data,
    ...
    if(IMEX_stage <= n_stages) {
      /*--- Since here we have just one 'src' vector, but we also need to deal with the current height and tracer,
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          const auto& zeta_s   = phi_zeta_curr.get_value(q);
          const auto& h_s      = zeta_s + zb_q;

          const auto& hu_s     = phi_hc_curr.get_value(q);
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          for(unsigned int s = 1; s <= IMEX_stage - 1; ++s) {
            ...
            flux += a[IMEX_stage - 1][s - 1]*dt*hc_s*(hu_s/(zeta_s + zb_q));
          }
      ...
      for(unsigned int cell = cell_range.first; cell < cell_range.second; ++cell) {
        ...
        for(unsigned int q = 0; q < phi.n_q_points; ++q) {
          ...
          for(unsigned int s = 1; s <= IMEX_stage - 1; ++s) {
            ...
            flux += b[s - 1]*dt*hc_s*(hu_s/(zeta_s + zb_q));
          }
      ...
      for(unsigned int face = face_range.first; face < face_range.second; ++face) {
        ...
        for(unsigned int q = 0; q < phi_m.n_q_points; ++q) {
          ...
          for(unsigned int s = 1; s <= IMEX_stage - 1; ++s) {
            ...
            const auto& avg_flux_s = 0.5*(hc_s_m*(hu_s_m/(zeta_s_m + zb_q)) + hc_s_p*(hu_s_p/(zeta_s_p + zb_q)));
            const auto& lambda_s   = std::max(std::abs(scalar_product(hu_s_m/(zeta_s_m + zb_q), n_plus)) +
                                              std::sqrt(EquationData::g*(zeta_s_m + zb_q)),
                                              std::abs(scalar_product(hu_s_p/(zeta_s_p + zb_q), n_plus)) +
                                              std::sqrt(EquationData::g*(zeta_s_p + zb_q)));
