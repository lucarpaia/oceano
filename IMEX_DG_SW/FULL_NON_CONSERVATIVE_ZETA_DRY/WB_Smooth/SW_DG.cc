...
void SWSolver<dim>::update_velocity() {
  ...
  /*--- Compute multigrid preconditioner for the velocity equation ---*/
  MGTransferMatrixFree<dim, float> mg_transfer;
...
std::tuple<double, double, double> SWSolver<dim>::compute_max_C_x_y() {
  ...
  FEValues<dim>               fe_values_u(fe_velocity, quadrature_elevation, update_values);
  std::vector<Vector<double>> solution_values_velocity(quadrature_elevation.size(), Vector<double>(dim));
...
void SWSolver<dim>::run(const bool verbose, const unsigned int output_interval) {
  ...
  while(std::abs(T - time) > 1e-10) {
    ...
    mg_transfer.build(dof_handler_elevation);
    mg_transfer.interpolate_to_mg(dof_handler_elevation, level_projection_zeta, zeta_curr);
    for(unsigned int level = 0; level < triangulation.n_global_levels(); ++level) {
      mg_matrices_SW[level].set_zeta_curr(level_projection_zeta[level]);
    }
    update_velocity();
