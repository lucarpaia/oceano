...
namespace EquationData {
  ...
  // We declare now the class that describes the initial condition for the height.
  //
  template<int dim>
  class Height: public Function<dim> {
  public:
    Height(const double initial_time = 0.0); /*--- Class constructor ---*/
  ...
  //
  template<int dim>
  Height<dim>::Height(const double initial_time): Function<dim>(1, initial_time) {}
  ...
  //
  template<int dim>
  double Height<dim>::value(const Point<dim>& p, const unsigned int component) const {
    (void)component;
