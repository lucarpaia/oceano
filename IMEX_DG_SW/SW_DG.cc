...
void SWSolver<dim>::update_height() {
  ...
  if(IMEX_stage == 2) {
    ...
    zeta_old.equ(1.0, zeta_tmp_3);
    cg.solve(SW_matrix, zeta_old, rhs_zeta, preconditioner);
  }
}
...
void SWSolver<dim>::update_discharge() {
  ...
  if(IMEX_stage == 2) {
    ...
    hu_old.equ(1.0, hu_tmp_3);
    cg.solve(SW_matrix, hu_old, rhs_hu, preconditioner);
  }
}
...
void SWSolver<dim>::output_results(const unsigned int step) {
  ...
  data_out.add_data_vector(dof_handler_height, zeta_old, "h", {DataComponentInterpretation::component_is_scalar});
